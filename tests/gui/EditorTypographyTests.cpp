...
// rollout-2026-07/firmament/out/VERDICT.md), so every engraved label is a
...
TEST_CASE ("Engraved labels brighten the dark plate where the raw render is clean", "[gui][typography]")
{
    ...
    const auto plate = juce::ImageCache::getFromMemory (BinaryData::plate_firmament_png,
                                                        BinaryData::plate_firmament_pngSize);
    ...
    CHECK (labelledControls == 12); // 9 knobs + 3 selectors (toggles are unlabeled per the master-05 family precedent, meter face is baked)
}
...
TEST_CASE ("PlateTypography renders glyphs and its offset pass on a flat ground", "[gui][typography]")
{
    ...
    juce::Image canvas (juce::Image::RGB, 160, 24, true);
    {
        ...
        typography.drawEngraved (g, "RESPONSE", canvas.getBounds().toFloat(), 1.0f, style);
    }
