...
// The factory-preset headroom gate (issue #42).
//
...
namespace
{
    ...
    // Thirteen factory presets ship - CMakeLists.txt's juce_add_binary_data
    ...
    //
    // ONE DELIBERATE DEVIATION FROM THE SUITE REFERENCE, and the only one in the
    // fleet: the two channels are DECORRELATED here. The suite reference is the
    // same programme in both channels, which for every other plugin is exactly
    // right - but Firmament is a stereo widener, and a signal with no side
    // component gives its entire mid/side path nothing to work on. Measured with
    // the mono reference, nine of the thirteen factory presets rendered at
    // precisely -12.000 dBFS: bit-transparent, i.e. the gate would have been
    // asserting that a pass-through does not clip. Since a widener's headroom
    // risk lives entirely in what it does to the side signal - a width boost
    // raises L-R, and L = M+S is what has to stay inside full scale - a mono
    // reference does not merely under-test this plugin, it tests the wrong thing.
    //
    // The decorrelation is the smallest one that fixes that: the right channel
    // plays the same four notes in the opposite order. Same synthesis, same
    // spectral content, same per-channel envelope, same peak normalisation - but
    // at every instant the two channels carry different notes, so the programme
    // is fully decorrelated and the side signal is as large as the mid. That is
    // the demanding case for a widener rather than an arbitrary one.
    //
                                                    ...
                                                    float peakDbfs = referencePeakDbfs)
    {
        ...
        for (int channel = 0; channel < 2; ++channel)
            for (int slot = 0; slot < noteCount; ++slot)
            {
                // The right channel plays the same four notes in the opposite
                // order - see the decorrelation note above.
                const auto noteIndex = channel == 0 ? slot : (noteCount - 1 - slot);
                const auto fundamental = notes[noteIndex];
                const auto start = slot * noteSamples;
                ...
                for (int sample = 0; sample < noteSamples; ++sample)
                {
                    ...
                    buffer.setSample (channel, start + sample, static_cast<float> (envelope * value));
                }
            }
