...
TEST_CASE ("PresetManager: current and legacy preset folders follow the platform convention", "[presets][branding]")
{
    ...
    REQUIRE (current == presetsRoot.getChildFile ("Basilica Audio").getChildFile ("Firmament"));
    REQUIRE (legacy == presetsRoot.getChildFile ("Yves Vogl").getChildFile ("Firmament"));
   #else
    ...
    REQUIRE (current == applicationData.getChildFile ("Basilica Audio")
                            .getChildFile ("Firmament").getChildFile ("Presets"));
