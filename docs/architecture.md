...
- **Auto Mono Safety** and **Haas Mode** (both bool parameters) are read once per block as an instant on/off gate, the same design already used for the bass-mono crossover's `lastBassMonoHz > 0.0f` gate in v0.1 - no separate smoothing, since these are rare, deliberate user actions rather than continuously-automated controls.
